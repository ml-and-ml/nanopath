...
| Config | Intervention | Comparison | Labless v2 |
|---|---|---|---:|
...
| `configs/spatial-rel-jepa.yaml` | Spatial relation JEPA with upstream compilation, fused AdamW and GPU augmentation | Published speed rerun | 0.671775 |
...
`configs/spatial-rel-jepa.yaml` now launches `spatial-rel-turbo` under `/data/$USER/nanopath/spatial-rel-upstream-20260915/full`, integrating upstream `07e3027`. It uses one H100, eight CPUs/eight training workers and prefetch two; validation keeps four workers, prefetch one, and 16 batches every 200 steps. Pillow-SIMD performs PIL crop/resize/flips, uint8 crops transfer to batched GPU photometrics, model modules compile in place, and AdamW uses its fused implementation. Local hue remains ±0.2 and participates in each view's random jitter order; upstream's default GPU transform omits this custom step. The spatial relation objective, seed, original tile pool, FINO targets, calibration and weighted v2 probes remain fixed. PIL rounding and GPU random streams change the exact training trajectory. Initial eager FLOP measurement includes GPU augmentation; startup and augmentation compilation are reported separately from training/validation/calibration and probes. The completed run scores **0.671775** on weighted v2 (−0.005166 versus `spatial-rel-fast`). Training, validation and calibration took **37.11 minutes**, down from 99.43 (62.7% less time; 2.68× speedup). Setup added 64.85 seconds, including 20.84 seconds of augmentation warmup; probes took 22.56 minutes, and the complete process took **61m 31s**. Mutation and progression account for most of the score decline. This remains an unvalidated seed-7777 result; the changed resizing and random streams prevent attributing the decline to a particular optimization. Source is `b703040`, with [W&B run `tvny273s`](https://wandb.ai/sophont-hm/nanopath/runs/tvny273s). The completion audit verifies 999,973 total presentations, 2.87610e17 FLOPs, all 39 validation passes, checkpoint/optimizer integrity and the unchanged 20-dataset evaluator. The run is local and has not been submitted to Labless.

The experiment lineage is our `spatial-rel-jepa` (`f6bf835`, weighted v2 0.677383) → `spatial-rel-fast` (`d502a155`, 0.676941) → `spatial-rel-turbo` (`b703040`, 0.671775) → four hyperparameter comparisons (`4ca2b6d`). “Turbo” names our spatial recipe with upstream performance changes integrated. Its spatial objective, model/readout, FINO settings, training seed and locked evaluator are retained; PIL/GPU augmentation and numerical execution change the exact trajectory. The sweep tunes that speed-integrated spatial implementation. Separate complete configs and outputs live under `/data/$USER/nanopath/spatial-hparams-20260915/`. All retain seed 7777, one H100/eight CPUs, 16 views, both public budget caps, calibration and the existing validation/probe protocol. Each changes one setting:

| Config / run name | Change from turbo | Weighted v2 | Delta vs turbo | Train/val/cal | Whole process |
| --- | --- | ---: | ---: | ---: | ---: |
| `configs/spatial-lr-cooldown.yaml` | Sample-budget LR cooldown; original peak and warmup, all other schedules fixed | 0.665993 | -0.005782 | 37m 45s | 63m 52s |
| `configs/spatial-lr-1e4.yaml` | Peak LR `0.0001`; original FLOP-based decay | 0.667445 | -0.004330 | 37m 38s | 63m 34s |
| `configs/spatial-lr-8e5.yaml` | Peak LR `0.00008`; original FLOP-based decay | 0.665920 | -0.005856 | 37m 42s | 64m 05s |
| `configs/spatial-rel-w02.yaml` | Spatial relation weight `0.2`; original LR | 0.670842 | -0.000934 | 38m 19s | 65m 25s |

All four completed and passed independent source, schedule, checkpoint, resource, budget and full weighted v2 audits. None exceeded turbo's 0.671775 in this discovery seed. Doubling relation weight was closest (−0.000934); both lower peak LRs and the complete cooldown scored lower. The cooldown reached exactly `1e-6` on the final optimizer update, confirming the intended schedule was exercised. Each used 999,973 tile presentations, 2.876098e17 FLOPs and all 39 validation passes. Training time includes validation and calibration; whole-process time additionally includes setup, probes, saves and logging. Execution source is `4ca2b6d`; jobs were 451231/451233/451235/451237, with records and audit receipts under the artifact root above. These are local, unvalidated results.

`train.compile`, `train.fused_adamw` and `train.gpu_augment` enable the upstream performance path in main, smoke and the spatial speed/sweep configs. Other historical experiment configs leave them disabled. The disabled augmentation path retains the previous tensor crop, NumPy HED and SciPy blur implementation. Use a current uv release supporting package-scoped dependency exclusions; `uv sync` installs the locked Kornia and Pillow-SIMD groups.
...
`configs/connected-jepa4.yaml` starts from spatial relation JEPA and samples 32 groups of four distinct eight-connected tiles per batch. A group stays within one slide; slides are sampled uniformly with replacement across groups. Global crop and flip geometry is shared within each group, while photometric augmentation and local views remain independent. Each tile's predictor receives the other three masked-student CLS tokens plus fixed sine/cosine encodings of their relative centers in source-tile strides, with signs transformed by the shared flips. Teacher patch targets remain detached. Every source tile receives the original per-tile losses; the three neighbor tokens add predictor work without extra backbone forwards. The predictor is discarded for the unchanged single-tile probes. The completed seed-7777 run scores 0.651808 on weighted v2, down 0.025575 versus spatial relation JEPA, mainly from classification, mutation, and robustness. This discovery run changes the coordinate pool, sampling, shared geometry, and prediction context together; matched controls are needed to attribute the decline.

`data.spatial_dir` points to a separate JPEG Parquet pool containing `path`, `jpeg`, and integer level-0 `x`, `y`, `spacing`. Preparation intersects existing NanoPath slides with local WSIs and the `224_0.50/coords` grids under `data.connected_source_dir`; it never reads stored pathology features. Equal slide quotas target four million tiles across many random connected regions of up to 64 tiles. Stored `x`/`y` are tile centers; extraction uses top-left `center - floor(spacing/2)`, approximately 0.5 MPP, BOX resizing to 224 pixels, and JPEG quality 95. Regions smaller than four tiles are discarded, so the realized pool can be below its target. A manifest records coordinate hashes and coverage; its hash is saved with the run. `data.dataset_dir` continues to supply the original FINO metadata and calibration images. Run `python prepare.py configs/connected-jepa4.yaml download=True` in a 16-CPU preparation allocation before training. Preprocessing time is separate from training time.

Connected groups remain flat batches of 128 source tiles: each occurrence counts once, including repeated tiles in later groups. The existing calibration reserve also remains within the one-million presentation cap. Context work is included in measured training FLOPs. Validation remains every 200 steps over 16 batches, with 16 training workers and four validation workers in a single-H100/16-CPU allocation. Setting `dino.jepa_neighbor_context: false` keeps connected sampling available for a context ablation.

Training now creates block masks with NumPy before transferring them, reuses each local DINO log-softmax across its two teacher targets, and bypasses calibrated-inference flag checks during pretraining. CPU augmentation uses NumPy stain math with contiguous CHW output and separable SciPy blur with the original random draws; each worker uses one BLAS thread. Validation accumulates losses on GPU before its final reduction. These changes preserve the objective and validation schedule; floating-point accumulation order can produce small gradient differences.

For the initial speed fixes, a controlled 16-CPU loader benchmark over 16,384 historical Parquet tiles improved from 157.9 to 186.1 tiles/s. Matched 400-step H100 checks, including two unchanged 16-batch validations, took 379.8 seconds before and 343.7 seconds after these changes (9.5% less wall time). Independent checks preserved mask values/weights and augmentation RNG; maximum normalized augmentation pixel error was 6.44e-6. The completed connected run took 109.5 minutes for training, validation, and calibration, plus 18.1 minutes for the full probes. That earlier connected run did not meet the one-hour training goal; the upstream spatial speed rerun above now completes training, validation and calibration in 37.11 minutes. Its reusable pool contains 3,999,511 tiles from 7,456 slides (125.2 GB); the CPU preparation job took 61.4 minutes. The completed run passed independent source, checkpoint, full-score, and accounting audits with 999,973 tile presentations and 2.87884e17 FLOPs.
...
By default, LR decay, weight decay, teacher-temperature, freeze, and KDE schedules are keyed to `train_flops / train.max_train_flops`; LR warmup is keyed to optimizer tile presentations. Huelocal reaches the sample cap before the FLOP cap, so the FLOP-keyed schedules intentionally stop early. Setting `dino.lr_sample_decay: true` changes only the post-warmup LR cosine to reach `dino.lr_min` on the last full optimizer batch allowed by the sample budget, excluding calibration's reserved presentations. Warmup and all other schedules retain their original behavior; both public budget caps stay fixed.
