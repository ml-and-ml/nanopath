...
Four hyperparameter comparisons start from `spatial-rel-turbo`, with separate complete configs and outputs under `/data/$USER/nanopath/spatial-hparams-20260915/`. All retain seed 7777, one H100/eight CPUs, 16 views, both public budget caps, calibration and the existing validation/probe protocol. Each changes one setting:

| Config / run name | Change from turbo |
| --- | --- |
| `configs/spatial-lr-cooldown.yaml` | Sample-budget LR cooldown; original peak and warmup, all other schedules fixed |
| `configs/spatial-lr-1e4.yaml` | Peak LR `0.0001`; original FLOP-based decay |
| `configs/spatial-lr-8e5.yaml` | Peak LR `0.00008`; original FLOP-based decay |
| `configs/spatial-rel-w02.yaml` | Spatial relation weight `0.2`; original LR |
