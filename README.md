...
`configs/spatial-rel-jepa.yaml` now launches the `spatial-rel-fast` runtime rerun with the tested speed fixes and training prefetch increased from two to four batches per worker. This gives the same 16 workers a larger queue to fill during validation; validation keeps four workers, prefetch one, and the original 200-step/16-batch cadence. The seed, tile pool, objective, and probe suite stay fixed. Its fresh output is `/data/$USER/nanopath/spatial-rel-fast-20260910/full`; the spatial-relation score above remains the original result until the rerun completes. The original 132.9-minute training/calibration time exceeds the two-hour maintainer reproduction limit, though public submissions have no wall-clock cap.
