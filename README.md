...
`configs/spatial-rel-jepa.yaml` now launches the `spatial-rel-fast` runtime rerun with the tested speed fixes, preserving the original seed, tile pool, objective, and validation schedule. Its fresh output is `/data/$USER/nanopath/spatial-rel-fast-20260910/full`; the spatial-relation score above remains the original result until the rerun completes. The original 132.9-minute training/calibration time exceeds the two-hour maintainer reproduction limit, though public submissions have no wall-clock cap.
